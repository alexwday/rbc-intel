"""Tests for shared vision pipeline."""
...
import ingestion.processors.vision as vision_module
from ingestion.processors.vision import (
    call_vision,
    open_rendered_pdf,
...
# ── open_rendered_pdf / render_page ─────────────────────────────


@patch("ingestion.processors.vision.fitz")
def test_open_rendered_pdf_success(mock_fitz):
    """Opens a PDF and exposes total pages for streaming rendering."""
    mock_doc = MagicMock()
...
@patch("ingestion.processors.vision.fitz")
def test_open_rendered_pdf_uses_render_lock(mock_fitz, monkeypatch):
    """Serializes MuPDF global error toggles during PDF open."""

    class DummyLock:
        """Track lock usage during rendering."""
    ...
    lock = DummyLock()
    monkeypatch.setattr(vision_module, "_RENDER_LOCK", lock)
...
@patch("ingestion.processors.vision.fitz")
def test_render_page_uses_render_lock(_mock_fitz, monkeypatch):
    """Serializes MuPDF global error toggles during page rendering."""
    ...
    rendered = RenderedPdf(Path("test.pdf"), mock_doc, "matrix", 1)
    assert render_page(rendered, 1) == b"png-data"
    assert lock.entered == 1
    assert lock.exited == 1
...
@patch("ingestion.processors.vision.time.sleep")
...
def test_call_vision_zero_retries_raises_runtime_error(monkeypatch):
    """A zero-retry configuration fails with a clear runtime error."""
    monkeypatch.setattr(vision_module, "_MAX_RETRIES", 0)
    mock_llm = MagicMock()
    prompt = _make_prompt()
...
@patch(
    "ingestion.processors.vision.shrink_image",
    return_value=b"small",
)
def test_process_page_half_dpi(_mock_shrink):
    """Falls back to half DPI when full and original fail."""
    mock_llm = MagicMock()
    mock_llm.call.side_effect = [
        ValueError("full failed"),
...
def test_process_page_falls_back_on_malformed_tool_response(_mock_shrink):
...
@patch(
    "ingestion.processors.vision.split_image",
