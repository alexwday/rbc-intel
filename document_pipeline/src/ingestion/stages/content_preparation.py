"""Stage 3: Content preparation for embedding and storage."""
...
from ..utils.content_chunker import chunk_content
from ..utils.content_types import (
    ContentPreparationResult,
    DenseTableDescription,
    PreparedDenseTable,
    PreparedPage,
)
from ..utils.dense_table_description import (
...
from ..utils.table_eda import run_table_eda, run_table_eda_from_region
...
def _reindex_chunks(chunks: list[Any], start_index: int) -> list[Any]:
    """Reassign chunk indexes to a page-global sequential order."""
...
def _prepare_dense_table_region(
    ...
    """Prepare one dense-table region for similarity and subretrieval."""
    ...
    chunks = chunk_content(replacement_content, llm, max_chunk_tokens)
    for chunk in chunks:
        chunk.is_dense_table_description = True
        chunk.routing_metadata = dict(routing_metadata)
...
def _prepare_page(
    ...
    Returns:
        PreparedPage with chunks and optional dense table data
    """
    ...
    if _is_dense_table_candidate(page):
        ...
        for dense_table in dense_tables:
            chunks.extend(
                _reindex_chunks(dense_table.chunks, next_chunk_index)
            )
            next_chunk_index += len(dense_table.chunks)

        if len(dense_tables) == 1:
            method = "dense_table_replaced"
            if len(chunks) > 1:
                method = "dense_table_replaced_chunked"
        else:
            method = "dense_tables_replaced"
            if any(
                len(dense_table.chunks) > 1 for dense_table in dense_tables
            ):
                method = "dense_tables_replaced_chunked"
    else:
        chunks = chunk_content(content, llm, max_chunk_tokens)
        if len(chunks) > 1:
            method = "chunked"
...
def _prepare_file(
    ...
    for page_data in pages_data:
        prepared = _prepare_page(page_data, llm, max_chunk_tokens)
        prepared_pages.append(prepared)

        dense_count += len(prepared.dense_tables)
        if not prepared.dense_tables and prepared.dense_table_eda is not None:
            dense_count += 1
    ...
    return ContentPreparationResult(
        ...
        dense_tables_processed=dense_count,
        pages_chunked=chunk_count,
    )
...
def run_content_preparation(llm: LLMClient) -> None:
    ...
    (dense table description + chunking), and writes per-file
    JSON results. Per-file failures are logged but do not
    crash the stage.
    ...
    max_chunk_tokens = get_content_prep_max_chunk_tokens()
    succeeded = 0
    failed = 0
    ...
    def _prepare(
        ...
        return _prepare_file(extraction, llm, max_chunk_tokens)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        ...
        for future in as_completed(future_to_ext):
            ...
            try:
                ...
                succeeded += 1
                logger.debug(
                    "Prepared %s — %d/%d dense, %d chunked",
                    filename,
                    result.dense_tables_processed,
                    len(result.pages),
                    result.pages_chunked,
                )
            except (
                ValueError,
                OSError,
