...
def _sheet_log_context(
    workbook_label: str,
    sheet_name: str,
    page_number: int | None = None,
    total_pages: int | None = None,
) -> str:
    """Build a concise workbook/sheet label for logs."""
    if page_number is not None and total_pages is not None:
        return (
            f"{workbook_label} sheet {page_number}/{total_pages} "
            f"'{sheet_name}'"
        )
    return f"{workbook_label} sheet '{sheet_name}'"
...
def call_vision(
    ...
    """Run XLSX vision extraction with retry."""
...
def _call_visual_prompt(
    ...
    """Call the visual-description prompt."""
    messages = []
...
def _call_visual_prompt_with_retry(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            wait = retry_delay * attempt
            logger.warning(
                "%s visual description retry %d/%d after %.1fs: %s",
                context or visual_label,
                attempt,
                max_retries,
                wait,
                exc,
...
def _describe_chart_visual(
    ...
    """Describe one worksheet chart."""
    region_id = f"visual_region_{visual_index}"
    anchor = _extract_anchor_position(getattr(chart, "anchor", None))
    context = f"sheet '{sheet_name}' chart {region_id}"
...
def _describe_image_visual(
    ...
    """Describe one embedded worksheet image."""
    region_id = f"visual_region_{visual_index}"
    ...
    page_title, content = call_vision(
        ...
        context=f"sheet '{sheet_name}' image {region_id}",
    )
    return {
        "region_id": region_id,
        "region_type": "visual",
...
def _build_visual_regions(
    ...
    """Build chart/image regions for one worksheet."""
...
def _build_region_table_lines(region: SheetRegion) -> list[str]:
    """Build one region-scoped markdown table with splice markers."""
    header_cells = [
        get_column_letter(column) for column in region.column_numbers
    ]
    lines = [
    ...
    for row_number, row_data in zip(region.row_numbers, region.rows):
        rendered_row = [
            _escape_table_text(row_data.get(column, ""))
            for column in region.column_numbers
        ]
        lines.append(f"| {row_number} | {' | '.join(rendered_row)} |")
    lines.append(f"<!-- region:{region.region_id} end -->")
    return lines


def _append_sheet_regions(
    lines: list[str],
    regions: list[SheetRegion],
) -> None:
    """Append region-scoped worksheet blocks in sheet order."""
    for index, region in enumerate(
        sorted(regions, key=lambda item: (item.min_row, item.min_col))
    ):
        if index > 0:
            lines.append("")
        lines.extend(_build_region_table_lines(region))
...
def _classify_sheet(
    ...
    """Classify one sheet for dense-table handling."""
    messages = []
    if prompt.get("system_prompt"):
        messages.append({"role": "system", "content": prompt["system_prompt"]})
    messages.append(
    ...
    response = llm.call(
        ...
        tools=prompt.get("tools"),
        tool_choice=prompt.get("tool_choice"),
        context=f"{context} classification".strip(),
    )
...
def _classify_sheet_with_retry(
    ...
    """Retry XLSX sheet classification."""
    max_retries = get_xlsx_classification_max_retries()
    retry_delay = get_xlsx_classification_retry_delay()
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            wait = retry_delay * attempt
            logger.warning(
                "%s classification retry %d/%d after %.1fs: %s",
                context or f"Sheet '{sheet_metadata['sheet_name']}'",
                attempt,
                max_retries,
                wait,
                exc,
...
def process_xlsx(file_path: str, llm: LLMClient) -> ExtractionResult:
    ...
    Example:
        ...
        "xlsx"
    """
    prompt = load_prompt("sheet_classification", _PROMPTS_DIR)
    ...
    try:
        for page_number, sheet in enumerate(sheets, start=1):
            ...
            if metadata["content_kind"] == "empty":
                ...
                metadata["page_type"] = "empty_sheet"
                pages.append(
            ...
            if metadata["content_kind"] == "visual_only":
                ...
                metadata["page_type"] = "visual_only_sheet"
                metadata["contains_chart"] = metadata.get("chart_count", 0) > 0
                pages.append(
                    _build_sheet_result(page_number, sheet, content, metadata)
                )
                continue
            ...
            metadata["contains_chart"] = metadata.get("chart_count", 0) > 0
            if metadata.get("handling_mode") == "dense_table_candidate":
                metadata["page_type"] = "dense_table_sheet"
            pages.append(
                _build_sheet_result(page_number, sheet, content, metadata)
            )
    finally:
