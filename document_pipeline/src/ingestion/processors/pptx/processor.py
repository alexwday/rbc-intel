...
def _slide_log_context(
    file_label: str,
    page_number: int,
    total_pages: int,
) -> str:
    """Build a concise filename/slide label for logs."""
    return f"{file_label} slide {page_number}/{total_pages}"
...
def process_page(
    ...
    context = _slide_log_context(file_label, page_number, total_pages)
...
def _classify_slide(
    ...
    Params:
        llm: LLMClient instance
        prompt: Loaded slide classification prompt
...
def _parse_slide_classification(
    ...
    if slide_type not in _VALID_PAGE_TYPES:
    ...
    return {
        ...
        "classification_rationale": str(rationale),
    }
    ...
    for attempt in range(1, max_retries + 1):
        try:
            return _classify_slide(
                llm, prompt, page_number, page_title, content, context
            )
            ...
            logger.warning(
                ...
                context or f"Slide {page_number}",
                attempt,
                max_retries,
                wait,
                exc,
...
def extract_slide_notes(pptx_path: str, slide_number: int) -> str:
    """Extract speaker notes from a PPTX slide via python-pptx.
    ...
    Returns:
        str — speaker notes text, empty string if none or on error
    """
    try:
...
def process_pptx(file_path: str, llm: LLMClient) -> ExtractionResult:
    ...
    Speaker notes are extracted via python-pptx and combined
    with programmatic PDF text as supplementary context.
    ...
    Example:
        ...
        12
    """
    extraction_prompt = load_prompt("slide_extraction", _PROMPTS_DIR)
    classification_prompt = load_prompt("slide_classification", _PROMPTS_DIR)
        ...
        with open_rendered_pdf(pdf_path, get_vision_dpi_scale()) as rendered:
            for page_num in range(1, rendered.total_pages + 1):
                try:
                    ...
                    supplementary = page_text
                    if notes:
                        supplementary = (
                            f"{page_text}\n\n" f"## Speaker Notes\n{notes}"
                            if page_text
                            else f"## Speaker Notes\n{notes}"
                        )
